...
/*
 * A square represents a position on the grid,
 * not an (x, y) pair.
 */
...
{
    return ((square)((y + BORDER_SIZE) * GRID_SIZE +
                     (x + BORDER_SIZE)));
}
...
{
    return (board_cell(board, _square) == CELL_EMPTY);
}

static inline bool
board_is_out(const Board *board, square _square)
{
    return (board_cell(board, _square) == CELL_OUT);
}
...
static inline cell
board_cell_at(const Board *board,
              square origin,
              square direction,
              int steps)
{
...
int     board_count_rays(const Board *board,
                         square _square,
                         square direction,
                         cell player,
                         int max_steps);

int     board_count_line(const Board *board,
                         square _square,
                         square axis,
                         cell player);

cell    board_winner(const Board *board);
bool is_legal_move(const Board *board, square pos, cell player);
